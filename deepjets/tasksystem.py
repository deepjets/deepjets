...
def asyncCall(func, name=None, must_exec=False):
    """
    ...
    If `must_exec` is set, the other process must `exec()` after the `fork()`.
...
class AsyncTask(object):
    ...
    def __init__(self, func, name=None, must_exec=False, env_update=None):
        """
        ...
        :param bool must_exec: if True, we do fork+exec, not just fork
        :param dict[str,str] env_update: for must_exec, also update these env vars
        """
        self.name = name or "unnamed"
        self.func = func
        self.must_exec = must_exec
        self.env_update = env_update
        self.parent_pid = os.getpid()
        proc_args = {
            "target": funcCall,
        ...
        if must_exec and sys.platform != "win32":
            self.Process = ExecingProcess
            self.Pipe = ExecingProcess_Pipe
            proc_args["env_update"] = env_update
        else:
            assert not env_update, "not supported if not must_exec"
    ...
    def _asyncCall(self):
        ...
        if not self.must_exec and sys.platform != "win32":
            global isFork
            isFork = True
        global isMainProcess
        isMainProcess = False
