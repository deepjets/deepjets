...
def plot_kinematics(signal, background, nbins=100,
    ...
    fig.suptitle(r'Pythia 8 $\sqrt{s}=13$ TeV', fontsize=16)

    pt_min, pt_max = pt_range
    mass_min, mass_max = mass_range

    vals1, _, _ = ax[0, 0].hist(signal_aux['pt_trimmed'][signal_selection], bins=np.linspace(pt_min, pt_max, nbins),
               histtype='stepfilled', facecolor='none', edgecolor='blue', normed=1,
               label=r'W jets', weights=signal_weights[signal_selection])
    vals2, _, _ = ax[0, 0].hist(background_aux['pt_trimmed'][background_selection], bins=np.linspace(pt_min, pt_max, nbins),
               histtype='stepfilled', facecolor='none', edgecolor='black', normed=1, linestyle='dotted',
    ...
    vals1, _, _ = ax[0, 1].hist(signal_aux['mass_trimmed'][signal_selection], bins=np.linspace(mass_min - 10, mass_max + 10, nbins),
               histtype='stepfilled', facecolor='none', edgecolor='blue', normed=1,
               label=r'W jets', weights=signal_weights[signal_selection])
    vals2, _, _ = ax[0, 1].hist(background_aux['mass_trimmed'][background_selection], bins=np.linspace(mass_min - 10, mass_max + 10, nbins),
               histtype='stepfilled', facecolor='none', edgecolor='black', normed=1, linestyle='dotted',
               label='QCD jets', weights=background_weights[background_selection])
    ...
    vals1, _, _ = ax[1, 1].hist(signal_aux['subjet_dr'][signal_selection], bins=np.linspace(0, 1.2, nbins),
               histtype='stepfilled', facecolor='none', edgecolor='blue', normed=1,
               label=r'W jets', weights=signal_weights[signal_selection])
    vals2, _, _ = ax[1, 1].hist(background_aux['subjet_dr'][background_selection], bins=np.linspace(0, 1.2, nbins),
               histtype='stepfilled', facecolor='none', edgecolor='black', normed=1, linestyle='dotted',
               label='QCD jets', weights=background_weights[background_selection])
